...
```{r linewidth=80}
readLines("https://raw.githubusercontent.com/cheriehua-cmu/36-350/main/README.md")
```
...
source_url("https://raw.githubusercontent.com/cheriehua-cmu/36-350/main/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/cheriehua-cmu/36-350/main/dark_and_stormy.R")
source_url("https://raw.githubusercontent.com/cheriehua-cmu/36-350/new_branch/dark_and_stormy.R")
```
...
```{r linewidth=80}
source_url("https://raw.githubusercontent.com/cheriehua-cmu/36-350-Fork/main/check_mark.R")
```
---
...
test_that(desc = "Tests if it works with uppercase letters", expect_type(f("A"), "double"))
test_that(desc = "Tests if it works with lowercase letters", expect_type(f("a"), "double"))
test_that(desc = "Tests for more than one character", expect_warning(f("test")))
test_that(desc = "Tests for non-character input", expect_error(f(8)))
test_that(desc = "Tests for multiple string inputs", expect_error(f("c", "a")))
```
...
f = function(letter) {
  if (nchar(letter) > 1) {
    warning("Only the first character will be used.")
  }
  if (typeof(letter) != "character") {
    stop("The input must be of type character.")
  }
  if (length(letter) > 1) {
    stop("Only one string must be input.")
  }
  return(log(which(letters==letter[1])))
}
