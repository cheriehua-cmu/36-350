...
```
```
The shorter vector is repeated to match the length of the longer vector, and the elements are multiplied pairwise.
```
...
```{r}
#answeer
s = c(setdiff(u, v), setdiff(v,u))
sort(s)
sum(is.element(c(u,v), s))
```
