...
charvec = unlist(words)
length(unique(charvec))
sort(table(charvec), decreasing = TRUE)[1:5]
```
...
regmatches(shakespeare.lines, regexpr(my.pattern, shakespeare.lines))
```

```{r linewidth=80}
unlist(regmatches(shakespeare.lines, gregexpr(my.pattern, shakespeare.lines)))
```

```
No, the gregexpr version has 91 occurences while the regexpr version has 89 occurences.
```
...
```{r linewidth=80}
words = strsplit(shakespeare.lines, " ")
charvec = unlist(words)
grep("[[:alnum:]][[:punct:]][[:alnum:]]|[[:punct:]][[:alnum:]]", charvec, value = TRUE)
```
...
```{r linewidth=80}
words = strsplit(date.lines, " ")
charvec = unlist(words)
input = grep("^[0-9]{4}/[0-9]{2}/[0-9]{2}", charvec, value = TRUE)
sort(table(input), decreasing = TRUE)[1:5]
```

```
The Tohoku earthquake and tsunami happened on 2011/03/11.
```
...
```{r linewidth=80}
geo.pattern = "[0-9]{1,3}\\.[0-9]{2}  [6-9]{1}\\.[0-9]{2}"
depth_mag = regmatches(date.lines, regexpr(geo.pattern, date.lines))
mags = as.numeric(substr(depth_mag, nchar(depth_mag)-4+1, nchar(depth_mag)))
depths = as.numeric(substr(depth_mag, 1, nchar(depth_mag) - 4))
mags[1:3]
depths[1:3]
```
...
dates = regmatches(sprint.lines, regexpr(date.pattern, sprint.lines))
years = as.numeric(substr(dates, nchar(dates)-4+1, nchar(dates)))
months = as.numeric(substr(dates, nchar(dates)-7+1, nchar(dates)-5))
table(years)
table(months)
```
...
```{r linewidth=80}
countries.pattern = "[A-Z]{3}"
countries = regmatches(sprint.lines, regexpr(countries.pattern, sprint.lines))
t = table(countries)
t
(t["USA"] + t["JAM"])/sum(t)
```
...
astro = regmatches(astro.lines, regexpr("<b>[0-9]{4}</b>", astro.lines))
counts = as.numeric(substr(astro, 4, 7))
